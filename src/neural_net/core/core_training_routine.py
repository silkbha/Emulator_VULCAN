...
from src.neural_net.dataset_utils import make_data_loaders
from src.neural_net.NN_utils import move_to, plot_core_y_mixs, weight_decay
...
class WeightedMSELoss(torch.nn.Module):
    '''
    Weighted logarithmic MSE loss function, designed to discount errors at very low values.
    
    CREDIT: Gijs Vermariën (PhD Candidate @ Leiden Observatory, Leiden University)
    '''
    def __init__(self):
    ...
    def forward(self, x_pred, x_true):
        with torch.no_grad():
            weight = torch.pow(10, 0.25 * torch.max(x_pred, x_true) + 5.0)
            weight[weight > 1] = 1.0
        return torch.mean(torch.mul(weight, torch.functional.mse_loss(x_pred, x_true, reduce=False)))

def loss_fn(x, y):
    '''
    Loss function to optimize neural network.
    '''
    loss = WeightedMSELoss.forward(x,y)
    return loss
